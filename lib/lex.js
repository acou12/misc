...
var isAlpha = function (char) {
    return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_".includes(char);
};
...
var lexSurround = function (surround, type, description) {
    return function (source, index, context) {
        var endQuoteIndex = index;
        if (source[index] === surround) {
            endQuoteIndex++;
            while (endQuoteIndex < source.length &&
                source[endQuoteIndex] !== surround) {
                endQuoteIndex++;
            }
            if (endQuoteIndex >= source.length) {
                console.log("unclosed ".concat(description, ". >:("));
                (0, process_1.exit)(1);
            }
            else {
                return {
...
var lexString = lexSurround('"', "string-literal", "string");
var lexRawId = function (source, index, context) {
    var surroundResult = lexSurround("`", "raw-id", "string")(source, index, context);
    if (surroundResult === undefined) {
        return undefined;
    }
    else {
        if (!surroundResult.tokens.every(function (token) {
            return token.type === "raw-id" &&
                token.value.split("").every(util_1.isOperator);
        })) {
            throw new Error("raw ids can only contain operator characters (".concat(util_1.ALLOWED_OPERATOR_SYMBOLS, ")"));
        }
        else {
